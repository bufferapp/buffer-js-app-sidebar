...
class PopoverButton extends PseudoClassComponent {
  ...
  render() {
    ...
    const hoveredOrActiveOrFocused =
      this.state.hovered || this.props.active || this.state.focused
    const hoveredOrClicked = this.state.hovered || this.state.clicked
    const hoverableIcon = React.cloneElement(this.props.icon, {
    ...
    const popoverOffset =
      this.props.popoverPosition === 'above'
        ? { top: -4, left: 3 }
        : { top: -3, left: 3 }
    ...
    return (
      ...
      >
        ...
        <div style={sizeStyles}>{hoverableIcon}</div>
      </ContainerElement>
    )
  }
  componentDidMount() {
